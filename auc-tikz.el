...
If matched, returns (face . (start . end)), otherwise returns nil."
  ...
  (if (char-equal (following-char) char)
	  ...
	  (progn
	    (forward-char 1)
	    (list `(,face . (,(1- (point)) . ,(point)))))
	(list `(,face . (,(point) . ,(1+ (point))))))
    nil))
...
;  (debug-prompt "Handling sequence " whichseq)
  (let ((matches nil))
    (catch 'fail
      (save-excursion
	(dolist (matcher args)
;	  (debug-prompt "Sequence matcher " matcher)
...
to matcher-list."
  (debug-prompt "Handling list " whichlist)
  (lexical-let ((start start)
    ...
    (matcher-alternates
     (list 
      "mainlist"
      (lambda () 
	(matcher-sequence whichlist
	 start
	 (if leading-sep 
	     '(lambda () (matcher-optional "leading sep" sep))
	   'matcher-noop)
	 elem
	 `(lambda () (matcher-repeat `(lambda () (matcher-sequence "sep-elem" sep elem))))
	 (if trailing-sep
	     `(lambda () (matcher-optional "trailing sep" sep))
	   'matcher-noop)
	 end)))
     (list "emptylist" (lambda ()
	(matcher-sequence "empty-list" start end))))
    ))

(defun matcher-key ()
  "Matches names of the form /a/b/c..., or a/b/.c"
  (matcher-repeat
   '(lambda () (matcher-alternates
		(list "keyword" (lambda () (matcher-regex "/?\\.[^])=,}]+" t 'font-lock-keyword-face)))
		(list "plainname" (lambda () (matcher-regex "/?[^])=,}/]+" t 'font-lock-variable-name-face)))))))
  ...
  (let ((start (point)))
	...
	(cond
	 ((char-equal (following-char) ?{)
	  (let ((brace (matcher-bracegroup face)))
...
(defun matcher-keyval (closer)
  ...
  (matcher-sequence "keyval"
   ;'(lambda () (matcher-regex "[^=,}]+" t 'font-lock-variable-name-face)) 
   'matcher-key
   'match-equal
   '(lambda () (matcher-alternates
		(list "tikzset" 'matcher-tikzset)
		(list "plainval" (lambda () (matcher-val closer 'font-lock-string-face)))
		))
   ))
...
(defun matcher-tikz-options (open close)
  ...
  (matcher-list 
   ...
   open
   '(lambda () (matcher-alternates
		(list "keyval" (lambda () (matcher-keyval close)))
		(list "plainname" (lambda () (matcher-regex "[^],)}]+" t 'font-lock-variable-name-face)))))
   'match-comma
   close
   t t
  ...
  (matcher-alternates
   (list 
    ...
    '(lambda ()
       (matcher-sequence
	"tikzset"
	'(lambda () (matcher-regex "\\\\tikzset" t 'font-lock-keyword-face))
	'matcher-tikzset)))
   (list 
    "tikz command" 
    '(lambda () 
      (matcher-sequence 
       "tikz command"
       '(lambda () (matcher-regex "\\\\[A-Za-z]+[[:space:]]*" t 'font-lock-keyword-face))
       '(lambda () 
	  (matcher-repeat
	   '(lambda ()
	      (matcher-alternates
	       (list "options" (lambda () (matcher-tikz-brackets)) 'match-open-bracket)
	       (list "nodename" (lambda () (matcher-parengroup 'font-lock-string-face t)) 'match-open-paren)
	       (list "bracegroup" (lambda () (matcher-bracegroup nil t)) 'match-open-brace)))))
       '(lambda () (matcher-char ?\; t 'font-lock-preprocessor-face)))))
   (list 
    "error case"
    '(lambda ()
      (matcher-sequence 
       "error"
       '(lambda () (matcher-regex "[^;]*" t 'font-lock-warning-face))
       '(lambda () (matcher-char ?\; t 'font-lock-preprocessor-face)))))))
		

(defun simple-setup
  (interactive)
  (matcher-process-font-locks (matcher-tikz-command)))
   ...
   ((and 
    ...
    (let* ((start (1- (overlay-get (overlay-get o-match 'start) 'delim-end)))
	   ...
	   (matches (save-excursion 
		      ...
		      (let ((temp (reverse (matcher-tikzset))))
